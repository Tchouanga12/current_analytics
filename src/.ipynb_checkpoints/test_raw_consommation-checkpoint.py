...
def test_clean_data(): 
	data = raw_consommation.load_data()
	data = raw_consommation.clean_data(data)
	...
	for column, expected_type in expected_types.items():
    		assert data[column].dtype == expected_type, f"Column '{column}' has unexpected data type. Expected: {expected_type}, Actual: {data[column].dtype}"
...
def test_save_data():
	data = raw_consommation.load_data()
	data = raw_consommation.clean_data(data)
	raw_consommation.save_data(data,'onde6')
